...
//      }
  ...
  void MergeLists(std::vector<T> &listA, std::vector<T> &listB,
    ...
    void SFC_3D_TreeSort(std::vector<T> &pNodes, double loadFlexibility,MPI_Comm comm) {
        ...
        unsigned int totalNumBuckets =1u << (3 * firstSplitLevel);
        ...
        while(!nodeStack.empty()) {
            ...
            {
                for(int i=0;i<NUM_CHILDREN_3D;i++)
                {
                    ...
                    //bucketCounts[8*levSplitCount+i]=(spliterstemp[hindexN] - spliterstemp[hindex]);
                    //bucketSplitter[8*levSplitCount+i]=spliterstemp[hindex];
            ...
            if (tmp.lev < pMaxDepth) {
                ...
                for (int i = 0; i < 8; i++) {
                    ...
                    }else if(tmp.lev<(firstSplitLevel-1))
                    {
                        ...
                        //bucketCounts[8*levSplitCount+i]=(spliterstemp[hindexN] - spliterstemp[hindex]);
                        //bucketSplitter[8*levSplitCount+i]=spliterstemp[hindex];
                        //bucketInfo[8*levSplitCount+i]=bucket;
                        bucketCounts.push_back((spliterstemp[hindexN] - spliterstemp[hindex]));
                        bucketSplitter.push_back(spliterstemp[hindex]);
                    ...
                    // Update the bucket info details.
                    if( tmp.lev == (firstSplitLevel-1) )
                    {
                        NodeInfo1<T> bucket(index, (tmp.lev + 1), spliterstemp[hindex], spliterstemp[hindexN]);
//                          bucketInfo.push_back(bucket);
//                          bucketInfo[8*levSplitCount+i]=bucket;
           ...
           /*  if(!rank)
            ...
            if(numLeafBuckets == totalNumBuckets) {
                ...
                for(int i=0;i<totalNumBuckets-1;i++)
                    {
                        //std::cout<<"Bucket Spliter : "<<i<<" : "<<bucketSplitter[i]<<std::endl;
...
#ifdef DEBUG_TREE_SORT
        if(!rank) std::cout<<"Initial Splitter Calculation ended "<<std::endl;
#endif
...
#endif
        for(int k=0;k<bucketCounts_g.size();k++){
            if(k==0)
        ...
        for(int i=0;i<splitBucketIndex.size()-1;i++)
       {
        ...
        while(!splitBucketIndex.empty()) {
            ...
            if (bucketInfo[splitBucketIndex[0]].lev < pMaxDepth) {
                ...
                NodeInfo1<T> tmp;
                // unsigned int numSplitBuckets = NUM_CHILDREN_3D * splitBucketIndex.size();
                ...
                for (int k = 0; k < splitBucketIndex.size(); k++) {
                    ...
                    seq::SFC_3D_Bucketting(pNodes, tmp.lev, pMaxDepth, tmp.rot_id, tmp.begin, tmp.end,splitterTemp, updateState);
                    ...
                    for (int i = 0; i < NUM_CHILDREN_3D; i++) {
                        ...
                        //newBucketCounts[8 * k + i] = (splitterTemp[hindexN] - splitterTemp[hindex]);
...
//                        newBucketSplitters[8 * k + i] = splitterTemp[hindex];
                ...
                MPI_Allreduce(&newBucketCounts[0], &newBucketCounts_g[0], newBucketCounts.size(), MPI_LONG_LONG, MPI_SUM,
                              comm);
                ...
                for (int k = 0; k < splitBucketIndex.size(); k++) {
                    ...
                    tmp = bucketInfo[splitBucketIndex[k]];
                    for (int i = 0; i < NUM_CHILDREN_3D; i++) {
                        bucketCount_gMerge.push_back(newBucketCounts_g[8 * k + i]);
                        bucketInfoMerge.push_back(newBucketInfo[8 * k + i]);
                        bucketSplitterMerge.push_back(newBucketSplitters[8 * k + i]);
